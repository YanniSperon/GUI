...
namespace GUI {
	...
	Text::Text(const std::string& fontPath, unsigned int fontSize, bool usePtSize, bool supportsMarkup, unsigned int numRows, unsigned int maxWidth, Alignment alignment, const char* fmt, ...)
		: m_Fonts(), m_NumberOfRows(numRows), m_MaxWidth(maxWidth == 0 ? INT_MAX : maxWidth), m_Scale(1.0f, 1.0f), m_Translation(0.0f, 0.0f), m_Min(FLT_MAX, FLT_MAX), m_Max(-FLT_MAX, -FLT_MAX), m_SupportsMarkup(supportsMarkup), m_Alignment(alignment), m_Color(1.0f, 1.0f, 1.0f, 1.0f), m_Shader(ShaderManager::GetInstance()->GetShader("Resources/Shaders/Text", SHADER_VERTEX_SHADER | SHADER_FRAGMENT_SHADER)), m_FontPath(fontPath), m_FontSize(fontSize), m_UsePtSize(usePtSize)
	{
	...
	void Text::Recalculate()
	{
		if (m_FontPath != "") {
			...
			if (m_SupportsMarkup) {
				...
				m_Fonts.clear();
				m_Fonts.push_back(FontManager::GetInstance()->GetFont(m_FontPath, m_FontSize, m_UsePtSize));
				FontManager::GetInstance()->MarkForCleanup();
	...
	void Text::SetTextAttributes(const std::string& text, const std::string& fontPath, int fontSize, int usePtSize, int supportsMarkup, int numRows, Alignment alignment)
	{
		switch (alignment)
		{
	...
	void Text::SetMaxWidth(unsigned int maxWidth)
	{
		if (m_MaxWidth != maxWidth) {
			m_MaxWidth = maxWidth == 0 ? INT_MAX : maxWidth;
			Recalculate();
		}
	}
